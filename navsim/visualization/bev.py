...
from navsim.visualization.config import BEV_PLOT_CONFIG, MAP_LAYER_CONFIG, AGENT_CONFIG, LIDAR_CONFIG, TRAJECTORY_CONFIG
...
def add_annotations_to_bev_ax_mask(
    ax: plt.Axes,
    annotations: Annotations,
    add_ego: bool = True,
    style_indices: Optional[np.ndarray] = None,
    style_list: Optional[List[Dict[str, Any]]] = None,
) -> plt.Axes:
    """
    Adds birds-eye-view visualization of annotations (ie. bounding boxes)
    :param ax: matplotlib ax object
    :param annotations: navsim annotations dataclass
    :param add_ego: whether to add ego bounding box, defaults to True
    :param style_indices: Optional integer array for non-ego boxes mapping into style_list
    :param style_list: Optional list of style dicts to merge with defaults per non-ego box
    :return: ax with plot
    """

    non_ego_idx = -1  # index over non-ego annotations to align with external indices
    for name_value, box_value in zip(annotations.names, annotations.boxes):
        ...
        # Default style for this agent type
        style_config = AGENT_CONFIG[agent_type].copy()

        if agent_type != TrackedObjectType.EGO:
            non_ego_idx += 1

            # Apply per-box overrides if provided via index mapping
            if style_indices is not None and style_list is not None and non_ego_idx < len(style_indices):
                idx = int(style_indices[non_ego_idx])
                if 0 <= idx < len(style_list):
                    style_config.update(style_list[idx])

        add_oriented_box_to_bev_ax(ax, agent_box, style_config)
...
def add_trajectory_to_bev_ax(
    ...
    traj_valid: Optional[np.ndarray] = None,
) -> plt.Axes:
    """
    ...
    :param start_pose: optional start pose
    :param traj_valid: optional trajectory validity mask
    :return: ax with plot
    """
    poses = None
    if isinstance(trajectory, Trajectory):
        traj_xy = trajectory.poses[:, :2]
        if start_pose is not None:
            poses = np.concatenate([start_pose[np.newaxis, :], traj_xy])
            if traj_valid is not None:
                mask = np.concatenate([np.ones((1,), dtype=bool), traj_valid], axis=0)
                poses = poses[mask]
        else:
            if traj_valid is not None:
                poses = traj_xy[traj_valid]
            else:
                poses = traj_xy
    else:
        # trajectory provided directly as ndarray
        traj_arr = np.asarray(trajectory)
        # reduce to XY if [T,3]
        if traj_arr.ndim == 2 and traj_arr.shape[1] >= 2:
            traj_xy = traj_arr[:, :2]
        else:
            traj_xy = traj_arr
        if traj_valid is not None:
...
def add_trajectory_to_bev_ax_config(
    ax: plt.Axes,
    trajectory: Trajectory | np.ndarray,
    config: TrajVisConfig,
) -> plt.Axes:
    """
    Add trajectory poses as lint to plot
    :param ax: matplotlib ax object
    :param trajectory: navsim trajectory dataclass
    :param config: TrajVisConfig object
    :return: ax with plot
    """
    for trajectory, color, alpha, width, zorder in zip(config.trajectories, config.colors, config.alphas, config.widths, config.zorder):
        ...
        trajectory_config["zorder"] = zorder
