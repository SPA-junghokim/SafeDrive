...
class MetricCacheProcessor:
    ...
    def compute_metric_cache(self, scenario: AbstractScenario) -> Optional[CacheMetadataEntry]:
        ...
        pred_states = get_trajectory_as_array(pdm_closed_trajectory, proposal_sampling, initial_ego_state.time_point)
        trajectory_states =pred_states[None]
        ...
        _drivable_area_map=self._pdm_closed._drivable_area_map
        _centerline=self._pdm_closed._centerline

        _geometries=_drivable_area_map._geometries
        coord_list=[]
        for polygon in _geometries:
            coordinates=polygon.__geo_interface__['coordinates'][0]
            coord=np.array(coordinates).astype(np.float32)
            coord_list.append(coord)
        _drivable_area_map._geometries=coord_list
        ...
        MetricCache(
            ...
            initial_ego_state,
            observation,
            _centerline._linestring,
            list(self._pdm_closed._route_lane_dict.keys()),
            _drivable_area_map,
            raw_progress
        ).dump()
        #total 40 observation._unique_objects
