...
class PDMScorer:
    ...
    def __init__(
        ...
        self._collision_tracked_tokens: Optional[List[List[str]]] = None
    ...
    def _reset(
        ...
        self._collision_tracked_tokens = [[] for _ in range(self._num_proposals)]
        self._drivable_area_violations = [None for _ in range(self._num_proposals)]
    ...
    def _calculate_ego_area(self) -> None:
        ...
        # a corner outside every drivable polygon puts the box out of bounds
        corners_in_drivable = corners_in_polygon[:, :, drivable_area_idcs]
        corner_drivable_mask = corners_in_drivable.sum(axis=2) > 0
        corner_out_of_bounds_mask = ~corner_drivable_mask.all(axis=-1)


        # per proposal, which future poses left the drivable area (consumed by pdm_score)
        for traj_id in range(corner_out_of_bounds_mask.shape[0]):
            mask = corner_out_of_bounds_mask[traj_id]              # (n_horizon,)
            if mask.any():
                self._drivable_area_violations[traj_id] = mask.tolist()[1:]
        ...
        for time_idx in range(self.proposal_sampling.num_poses + 1):
            ...
            for proposal_idx, geometry_idx in zip(intersecting[0], intersecting[1]):
                ...
                if collisions_at_stopped_track_or_active_front or (
                    ...
                    # Save tokens if collision is at fault
                    if tracked_object.tracked_object_type in AGENT_TYPES and token not in self._collision_tracked_tokens[proposal_idx]:
                        self._collision_tracked_tokens[proposal_idx].append(token)
