...
class PDMScorerConfig:
    ...
    # EPDMS-only (navsim v2 / hydraMDP++); inert in pdms mode
    lane_keeping_weight: float = 2.0
    lane_keeping_deviation_limit: float = 0.5   # [m] lateral deviation limit
    lane_keeping_horizon_window: float = 2.0    # [s] continuous-exceed window
...
class PDMScorer:
    ...
    def __init__(
        ...
        # EPDMS-only extra metrics (stored outside the v1 enum arrays so pdms mode is untouched)
        self._tlc_scores: Optional[npt.NDArray[np.float64]] = None
        self._lk_scores: Optional[npt.NDArray[np.float64]] = None
    ...
    def score_proposals(
        ...
        :param scoring_mode: "pdms" (v1, default) | "epdms" (v2: adds TLC+LK, DDC/TLC as multipliers)
        :return: array containing the final score of each proposal
        """
        ...
        if scoring_mode == "epdms":
            # v2 EPDMS: DDC/TLC become multipliers, LK is weighted, and EP is DECOUPLED from the
            self._calculate_traffic_light_compliance()
            self._calculate_lane_keeping()
            return self._aggregate_epdms_scores()
    ...
    def _aggregate_epdms_scores(self) -> npt.NDArray[np.float64]:
        """EPDMS final = (NC·DAC·DDC·TLC) · (w_ep·EP + w_ttc·TTC + w_lk·LK)/Σw.
        v2 (pdm_scorer._aggregate_pdm_scores): DDC/TLC are multipliers, LK weighted, and EP is
        DECOUPLED from the penalties — the penalty mask only sets the normalization constant, the
        numerator is un-masked raw progress, so a colliding/off-road proposal keeps its true EP."""
        nc = self._multi_metrics[MultiMetricIndex.NO_COLLISION]
        dac = self._multi_metrics[MultiMetricIndex.DRIVABLE_AREA]
        ddc = self._weighted_metrics[WeightedMetricIndex.DRIVING_DIRECTION]
        mult = nc * dac * ddc * self._tlc_scores

        masked_progress = self._progress_raw * mult                     # penalty mask -> norm constant only
        norm_constant = float(np.max(masked_progress)) if masked_progress.size else 0.0
        if norm_constant > self._config.progress_distance_threshold:
            ep = np.clip(self._progress_raw / norm_constant, 0.0, 1.0)   # un-masked raw progress in numerator
        else:
            ep = np.ones(self._num_proposals, dtype=np.float64)
        self._weighted_metrics[WeightedMetricIndex.PROGRESS] = ep        # epdms EP column = v2 decoupled EP

        # ttc exception marker (2.0, set when _calculate_ttc raised) -> treat as 1.0 (safe) in aggregation
        ttc = np.where(self._weighted_metrics[WeightedMetricIndex.TTC] == 2.0, 1.0,
                       self._weighted_metrics[WeightedMetricIndex.TTC])
        w_ep, w_ttc, w_lk = (self._config.progress_weight, self._config.ttc_weight,
                             self._config.lane_keeping_weight)
        weighted = (w_ep * ep + w_ttc * ttc + w_lk * self._lk_scores) / (w_ep + w_ttc + w_lk)
        return mult * weighted

    def _calculate_traffic_light_compliance(self) -> None:
        """navsim v2 / hydraMDP++ TLC (multiplier): 0 if ego intersects a red-light stop area.
        Stored in self._tlc_scores (no v1 enum slot)."""
        tlc_scores = np.ones(self._num_proposals, dtype=np.float64)
        for time_idx in range(self.proposal_sampling.num_poses + 1):
            ...
            for proposal_idx, geometry_idx in zip(intersecting[0], intersecting[1]):
                if tlc_scores[proposal_idx] == 0.0:
                    continue
                token = self._observation[time_idx].tokens[geometry_idx]
                if token.startswith(self._observation.red_light_token):
                    tlc_scores[proposal_idx] = 0.0
        self._tlc_scores = tlc_scores

    def _calculate_lane_keeping(self) -> None:
        """navsim v2 / hydraMDP++ LK (weighted): 0 if lateral deviation from centerline exceeds
        the limit for >= horizon_window consecutive steps (intersections skipped).
        Stored in self._lk_scores (no v1 enum slot)."""
        lk_scores = np.ones(self._num_proposals, dtype=np.float64)
        lateral_deviation_limit = self._config.lane_keeping_deviation_limit
        continuous_steps_required = int(np.ceil(
            self._config.lane_keeping_horizon_window / self.proposal_sampling.interval_length))
        centerline = getattr(self._centerline, "linestring", self._centerline)
        for proposal_idx in range(self._num_proposals):
            ...
            for time_idx in range(self.proposal_sampling.num_poses + 1):
                ...
                if self._drivable_area_map.is_in_layer(ego_position, layer=SemanticMapLayer.INTERSECTION):
                    continue
                if ego_position.distance(centerline) > lateral_deviation_limit:
                    consecutive_exceeds += 1
                else:
                    consecutive_exceeds = 0
                if consecutive_exceeds >= continuous_steps_required:
                    lk_scores[proposal_idx] = 0.0
                    break
        self._lk_scores = lk_scores
    ...
    def _calculate_no_at_fault_collision(self) -> None:
        ...
        # (token, time_idx) per at-fault hit — at-fault tokens re-enter every intersecting step
        self.proposal_fault_collision_times = {
            proposal_idx: [] for proposal_idx in range(self._num_proposals)
        }
        ...
        for time_idx in range(self.proposal_sampling.num_poses + 1):
            ...
            for proposal_idx, geometry_idx in zip(intersecting[0], intersecting[1]):
                ...
                if collisions_at_stopped_track_or_active_front or (
                    ...
                    self.proposal_fault_collision_times[proposal_idx].append((token, time_idx))
    ...
    def _calculate_driving_direction_compliance(self) -> None:
        ...
        # remove intersection (navsim v2 EPDMS fix: oncoming progress made while the
        # ego is inside an intersection must not count toward a direction violation,
        # since lane travel direction is undefined there)
        for proposal_idx in range(self._num_proposals):
            for time_idx in range(self.proposal_sampling.num_poses + 1):
