...
class Lidar:
    ...
    def from_paths(cls, sensor_blobs_path: Path, lidar_path: Path, sensor_names: List[str]) -> Lidar:
        ...
        if "lidar_pc" in sensor_names and lidar_path != None:
...
class Scene:
    ...
    def get_future_trajectory(self, num_trajectory_frames: Optional[int] = None, start_frame_idx : Optional[int] = None) -> Trajectory:
        """
        Extracts future trajectory of the human operator in local coordinates (ie. ego rear-axle).
...
class PDMResults_Pairwise:
    ...
    collision_tokens: str
    collision_time_idcs: List[float]
    drivable_area_compliance_timestep: Optional[List[Optional[Dict[str, Any]]]] = None
