...
class MetricCacheLoader:
    ...
    def _load_metric_cache_paths(self, cache_path: Path) -> Dict[str, Path]:
        ...
        # metadata records absolute paths, which are stale if the cache was built elsewhere
        cache_paths = [str(cache_path / p.split(f"/{cache_path.name}/", 1)[-1])
                       if f"/{cache_path.name}/" in p else p for p in cache_paths]
        metric_cache_dict = {p.split("/")[-2]: p for p in cache_paths}
